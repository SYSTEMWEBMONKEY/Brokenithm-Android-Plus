...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val editServer = findViewById<EditText>(R.id.edit_server).apply {
            ...
            hint = "Server (e.g. 192.168.1.1 or [ipv6]:port)" // IPv6 输入提示
        }

        findViewById<Button>(R.id.button_start).setOnClickListener {
            var server = editServer.text.toString()
            if (server.isBlank()) {
            ...
            if (server.contains(":") && !server.startsWith("[")) {
                if (server.contains("]")) {
                    ...
                    if (parts.size >= 2) {
                        ...
                        if (lastPart.all { it.isDigit() }) {
                            val ipPart = parts.dropLast(1).joinToString(":")
                            server = "[$ipPart]:$lastPart"
                        } else {
                            server = "[$server]"
                        }
                    } else {
                ...
                Toast.makeText(
                    this,
                    "Auto-corrected to IPv6 format: $server",
                    Toast.LENGTH_LONG
                ).show()
            }
            ...
            if (mExitFlag) {
                ...
                if (address == null) {
                    val errorMsg = if (server.contains(":")) {
                        "Invalid IPv6 format. Use [ipv6-address]:port"
                    } else {
                ...
                // 显示连接信息
                val connectionInfo = "Connecting to ${address.hostString}:${address.port}"
    ...
    private fun parseAddress(address: String): InetSocketAddress? {
        // 处理IPv6带方括号格式: [240e:399:501c:8df0:5a11:22ff:fe0b:b9a8]:52468
        if (address.startsWith("[") && address.contains("]:")) {
            val endBracket = address.indexOf(']')
            if (endBracket != -1) {
                val ip = address.substring(1, endBracket)
                val portPart = address.substring(endBracket + 2) // 跳过"]:"
                val port = portPart.toIntOrNull() ?: serverPort
                return try {
                    InetSocketAddress(ip, port)
                } catch (e: IllegalArgumentException) {
                    Log.e(TAG, "Invalid IPv6 address format: $address", e)
                    null
                }
            }
        }
    ...
    private fun initTasks() {
        receiverTask = AsyncTaskUtil.AsyncTask.make(
            doInBackground = {
                ...
                if (mTCPMode) {
                    ...
                    val socket = try {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                            DatagramSocket(null).apply {
                                ...
                                Log.d(TAG, "UDP socket bound to IPv6 wildcard")
                            }
                        } else {
                            DatagramSocket(serverPort).apply {
                                reuseAddress = true
                                Log.d(TAG, "UDP socket bound to port $serverPort (legacy)")
                            }
                        }
                        ...
                        runOnUiThread {
                            Toast.makeText(
                                this@MainActivity,
                                "Failed to bind UDP port: ${e.message}",
                                Toast.LENGTH_LONG
                            ).show()
                        }
                        return@make
                    ...
                    Log.d(TAG, "UDP receiver started for ${address.hostString}:${address.port}")

                    while (!mExitFlag) {
                        try {
                            socket.receive(packet)
                            val packetAddress = packet.address
                            val packetPort = packet.port

                            if (packetAddress == address.address && packetPort == address.port) {
                                ...
                                Log.w(TAG, "Received packet from unexpected source: " +
                                        "${packetAddress?.hostAddress}:$packetPort")
                            }
        ...
        senderTask = AsyncTaskUtil.AsyncTask.make(
            doInBackground = {
                ...
                Log.d(TAG, "Attempting to connect to ${address.hostString}:${address.port}")
                runOnUiThread {
                    Toast.makeText(
                        this@MainActivity,
                        "Connecting to ${address.hostString}:${address.port}...",
                        Toast.LENGTH_SHORT
                    ).show()
                }
                ...
                if (mTCPMode) {
                    try {
                        ...
                        if (address.address is Inet6Address) {
                            Log.d(TAG, "Creating IPv6 TCP socket")
                            mTCPSocket = Socket().apply {
                                // 设置连接超时
                                connect(address, 5000)
                            }
                        } else {
                            Log.d(TAG, "Creating IPv4 TCP socket")
                            mTCPSocket = Socket().apply {
                                connect(address, 5000)
                            }
                        }

                        mTCPSocket.tcpNoDelay = true
                        Log.d(TAG, "TCP connected successfully")

                        runOnUiThread {
                            Toast.makeText(
                                this@MainActivity,
                                "Connected to ${address.hostString}:${address.port}",
                                Toast.LENGTH_SHORT
                            ).show()
                        }
                        ...
                        // 重置UI状态
                        runOnUiThread {
                            findViewById<Button>(R.id.button_start).setText(R.string.start)
                            findViewById<EditText>(R.id.edit_server).isEnabled = true
    ...
    private fun getLocalIPAddress(preferIPv4: Boolean = false): ByteArray {
        try {
            ...
            for (intf in interfaces) {
                ...
                for (addr in addrs) {
                    ...
                    when (addr) {
                        is Inet4Address -> {
                            if (preferIPv4) return addr.address
                        }
                        is Inet6Address -> {
                            // 跳过本地链路地址 (fe80::/10)
                            if (!addr.isLinkLocalAddress && !preferIPv4) {
                                Log.d(TAG, "Found IPv6 address: ${addr.hostAddress}")
                                return addr.address
                            }
                        }
                    }
    ...
    private fun sendConnect(address: InetSocketAddress?) {
        ...
        thread {
            // 根据目标地址类型选择本地地址类型
            val isIPv6 = address.address is Inet6Address
            val selfAddress = getLocalIPAddress(!isIPv6)

            if (selfAddress.isEmpty()) {
                Log.e(TAG, "Failed to get local IP address for ${if (isIPv6) "IPv6" else "IPv4"}")
                runOnUiThread {
                    Toast.makeText(
                        this@MainActivity,
                        "Failed to get local ${if (isIPv6) "IPv6" else "IPv4"} address",
                        Toast.LENGTH_LONG
                    ).show()
                }
                return@thread
            ...
            Log.d(TAG, "Using local address: ${InetAddress.getByAddress(selfAddress).hostAddress}")

            val buffer = ByteArray(21)
            byteArrayOf('C'.byte(), 'O'.byte(), 'N'.byte()).copyInto(buffer, 1)
            ByteBuffer.wrap(buffer)
                .put(4, if (selfAddress.size == 4) 1.toByte() else 2.toByte()) // 1=IPv4, 2=IPv6
            ...
            try {
                ...
                socket.send(packet)
                socket.close()
                Log.d(TAG, "Sent CONNECT to ${address.hostString}:${address.port}")
            } catch (e: Exception) {
                Log.e(TAG, "Failed to send CONNECT", e)
                runOnUiThread {
                    Toast.makeText(
                        this@MainActivity,
                        "Failed to send connection request: ${e.message}",
                        Toast.LENGTH_LONG
                    ).show()
                }
            }
